...
- (UIView *)carousel:(iCarousel *)carousel viewForItemAtIndex:(NSInteger)index reusingView:(UIView *)view
{
    ...
    if (nil == view)
    {
        ...
        {
            if (_carouselView.frame.size.width > IPAD_PORTRAIT_SCREEN_WIDTH)
            {
                refWidth = _carouselView.frame.size.height;
            }
            else
            {
                refWidth = _carouselView.frame.size.width;
...
- (CGFloat)carousel:(iCarousel *)carousel valueForOption:(iCarouselOption)option withDefault:(CGFloat)value
{
    ...
    {
        return (IS_IPAD || IS_IPHONE_6P ? value * 1.5 : (IS_IPHONE_4_OR_LESS ? value * 1.9 : value * 1.25));
    }
    else if (option == iCarouselOptionRadius)
    {
        return (IS_IPAD ? value * 1.75 : value * 1.0);
    }
...
-(void)pushViewController:(NSInteger)index
{
    ...
    else if ([keyID isEqual:GLUCOSE_SERVICE_UUID])
    {
        ...
        {
            UIAlertView *gtAlert = [[UIAlertView alloc] initWithTitle:LOCALIZEDSTRING(@"unknownServiceAlert") message:LOCALIZEDSTRING(@"goToGattDBAlert" ) delegate:self cancelButtonTitle:@"Yes" otherButtonTitles:@"Cancel", nil];
            [gtAlert show];
        }
    }
 ...
 *  @method AlertView:
 *
 *  @discussion	 Alert the user that app redirecting to GATTDB view
 *
 */
        ...
        [capsenseServiceModel startDiscoverCharacteristicWithUUID:nil completionHandler:^(BOOL success, CBService *service, NSError *error)
        {
           if (success)
           {
               ...
               for (CBCharacteristic *characteristic in service.characteristics)
               {
                   ...
                   [carouselCharacteristics addObject:characteristic];
               }
               ...
               {
                   [carouselArray addObjectsFromArray:(NSArray*)tempCarouselArray];
                   [carouselServices addObject:service];
                   [_carouselView reloadData];
               }
           }
...
-(void)deviceOrientationDidChange:(NSNotification *)notification
{
    ...
    {
        emptyServiceLabel.frame = CGRectMake((self.view.frame.size.width/2)-(EMPTY_SERVICE_LABEL_WIDTH/2), (self.view.frame.size.height/2)-(EMPTY_SERVICE_LABEL_HEIGHT/2), EMPTY_SERVICE_LABEL_WIDTH, EMPTY_SERVICE_LABEL_HEIGHT);
    }
}
