 ...
 *
 */
@interface GATTDBDescriptorDetailsViewController () <cbCharacteristicManagerDelegate,UIAlertViewDelegate>
{
    /* Datafields  */
...
-(void)peripheral:(CBPeripheral *)peripheral didUpdateValueForDescriptor:(CBDescriptor *)descriptor error:(NSError *)error
{
    if (error == nil)
    {
        ...
        {
            [self parseDataForCharacteristicPresentationFormatDescriptor:descriptor];
        ...
        else if ([descriptor.UUID.UUIDString isEqual:CBUUIDCharacteristicUserDescriptionString])
        {
            ...
            descriptorHexValueLabel.text = [NSString stringWithFormat:@"%@",data];
...
-(void) parseDataForCharacteristicPresentationFormatDescriptor:(CBDescriptor *)descriptor
{
    NSData *descriptorData = descriptor.value;
    const uint8_t *dataPointer = [descriptorData bytes];
    NSUInteger offset = 0;
    
    NSString *hexValue = [NSString stringWithFormat:@"%@",descriptor.value];
    descriptorHexValueLabel.text = hexValue;
    
    int formatValue = dataPointer[offset];
    
    offset++;
    uint8_t exponentValue = dataPointer[offset];
    
    // Finding unit value
    offset++;
    uint16_t unitValue = 0;
    unitValue = CFSwapInt16LittleToHost(*(uint16_t *)&dataPointer[offset]);
    
    // finding the name space value
    offset = offset + 2;
    uint8_t namespaceValue = dataPointer[offset];
    NSString *nameSpace;
    ...
    descriptionValue = CFSwapInt16LittleToHost(*(uint16_t *)&dataPointer[offset]);
    
    
    NSString *descriptorValueInfo = [NSString stringWithFormat:@"Format = %@ \nExponent = %d\nUnit = %d \nNamespace = %@ \nDescription =%d",
                                     [self getEnumerationForformatValue:formatValue],
                                     exponentValue,unitValue,nameSpace,descriptionValue];
    descriptorValueLabel.text = descriptorValueInfo;
}
