...
-(void) initSensorHubmodel
{
    ...
    [mSensorHubModel startDiscoverAccelerometerCharacteristicsWithHandler:^(BOOL success, NSError *error) {
        ...
        if (success)
        {
            isAccelerometerCharacteristicsdiscovered = YES;
            
            [mSensorHubModel startUpdateAccelerometerXYZValueswithHandler:^(BOOL success, NSError *error) {
                
                if (success)
                {
                    @synchronized(mSensorHubModel.accelerometer){
                        accellermeterReadingXValueLbl.text = [NSString stringWithFormat:@"%.0f",mSensorHubModel.accelerometer.xValue];
                        accellermeterReadingYValueLbl.text = [NSString stringWithFormat:@"%.0f",mSensorHubModel.accelerometer.yValue];
                        accellermeterReadingZValueLbl.text = [NSString stringWithFormat:@"%.0f",mSensorHubModel.accelerometer.zValue];
                        
                        [self handleAccelerometerGraph];
                    }
                }
            }];
        }
    ...
    [mSensorHubModel startDiscoverBarometerCharacteristicsWithHandler:^(BOOL success, NSError *error) {
        ...
        if (success)
        {
            isBarometerCharacteristicsdiscovered = YES;
            
            [mSensorHubModel startUpdateBarometerPressureValueWithHandler:^(BOOL success, NSError *error) {
                
                if (success)
                {
                    @synchronized(mSensorHubModel.barometer)
                    {
                        pressureReadingValueLbl.text = mSensorHubModel.barometer.pressureValueString;
                        [self handlePressureGraph];
                    }
                }
            }];
        }
    ...
    [mSensorHubModel startDiscoverTemperatureCharacteristicsWithHandler:^(BOOL success, NSError *error) {
        ...
        if (success)
        {
            isTemperatureCharacteristicsdiscovered = YES;
            
            [mSensorHubModel startUpdateTemperatureValueCharacteristicWithHandler:^(BOOL success, NSError *error) {
                
                if (success)
                {
                    @synchronized(mSensorHubModel.temperatureSensor){
                        
                        temperatureReadingValueLbl.text = mSensorHubModel.temperatureSensor.temperatureValueString;
                        [self handleTemperatureGraph];
                    }
                }
            }];
        }
        ...
        [mSensorHubModel readValuesForAccelerometerCharacteristicsWithHandler:^(BOOL success, NSError *error)
         {
             if (success)
             {
                 if (mSensorHubModel.accelerometer.sensorTypeString != nil)
                 {
                     accelerometerSensorTypeValueLbl.text = mSensorHubModel.accelerometer.sensorTypeString;
                 }
                 if (mSensorHubModel.accelerometer.scanIntervalString != nil)
                 {
                     accelerometerSensorScanIntervalTxtFld.text = mSensorHubModel.accelerometer.scanIntervalString;
                 }
             }
         }];
    }
...
-(void) readTemperatureCharacteristics
{
    if (isTemperatureCharacteristicsdiscovered)
    {
        [mSensorHubModel readValuesForTemperatureCharacteristicsWithHandler:^(BOOL success, NSError *error) {
            ...
            if (success)
            {
                if (mSensorHubModel.temperatureSensor.sensorScanIntervalString != nil)
                {
                    temperatureSensorScanIntervalTxtFld.text = mSensorHubModel.temperatureSensor.sensorScanIntervalString;
                }
                
                if (mSensorHubModel.temperatureSensor.sensorTypeString != nil)
                {
                    temperatureSensorTypeValueLbl.text = mSensorHubModel.temperatureSensor.sensorTypeString;
                }
            }
        }];
    }
...
-(void) initBatteryModel
{
    ...
    [mSensorHubModel startDiscoverBatteryCharacteristicsWithHandler:^(BOOL success, NSError *error) {
        ...
        if (success)
        {
            [mBatteryModel startUpdateCharacteristic];
        }
    }];
}
...
-(void) readBarometerCharacteristics
{
    if (isBarometerCharacteristicsdiscovered)
    {
        [mSensorHubModel readValuesForBarometerCharacteristicsWithHandler:^(BOOL success, NSError *error)
        {
            if (success)
            {
                if (mSensorHubModel.barometer.sensorTypeString != nil)
                {
                    pressureSensorTypeValueLbl.text = mSensorHubModel.barometer.sensorTypeString;
                }
                if (mSensorHubModel.barometer.sensorScanIntervalString != nil)
                {
                    pressureSensorScanIntervalTxtFld.text = mSensorHubModel.barometer.sensorScanIntervalString;
                }
            }
        }];
    }
...
- (IBAction)locateDeviceBtnClicked:(UIButton *)sender
{
    [mSensorHubModel startDiscoverImmediateAlertCharacteristicsWithHandler:^(BOOL success, NSError *error) {
        ...
        if (success)
        {
            [mSensorHubModel.findMeModel updateImmedieteALertCharacteristicValue:kHighAlert WithHandler:^(BOOL success, NSError *error) {
